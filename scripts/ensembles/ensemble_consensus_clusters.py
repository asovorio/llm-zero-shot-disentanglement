...
It keeps only those conversations all 3 models agree on.
...
def build_consensus(
    ...
    for cid in ordered_chunk_ids:
        ...
        consensus_sets = set(cluster_sets_b) & cluster_sets_s & cluster_sets_f
    ...
    logger.info("Wrote consensus ensemble predictions to %s", output_path)
    return output_path


def main():
    ap = argparse.ArgumentParser(description="Consensus ensemble over cluster assignments.")
    ...
    split = cfg.run.split
    output_path = Path(cfg.paths.results_dir) / "consensus" / split / "predictions.jsonl"
